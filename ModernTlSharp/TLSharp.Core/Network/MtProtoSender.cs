using ModernTlSharp.TLSharp.Core.Exceptions;
using ModernTlSharp.TLSharp.Core.MTProto;
using ModernTlSharp.TLSharp.Core.MTProto.Crypto;
using ModernTlSharp.TLSharp.Core.Network.Exceptions;
using ModernTlSharp.TLSharp.Core.Network.Requests;
...
namespace ModernTlSharp.TLSharp.Core.Network
{
    public class MtProtoSender
    {
        ...
        public async Task<byte[]> Receive(TLMethod request, CancellationToken token = default(CancellationToken))
        {
            while (!request.ConfirmReceived)
            {
                ...
                {
                    _ = await processMessageAsync(result.Item2, result.Item3, messageReader, request, token);
                }
        ...
        private async Task<bool> processMessageAsync(ulong messageId, int sequence, BinaryReader messageReader, TLMethod request, CancellationToken token = default(CancellationToken))
        {
            token.ThrowIfCancellationRequested();
        ...
        private async Task<bool> HandleGzipPackedAsync(ulong messageId, int sequence, BinaryReader messageReader, TLMethod request, CancellationToken token = default(CancellationToken))
        {
            token.ThrowIfCancellationRequested();
            ...
            using (var ms = new MemoryStream())
            {
                ...
                {
                    _ = await processMessageAsync(messageId, sequence, compressedReader, request, token);
                }
            }
        ...
        private async Task<bool> HandleBadServerSaltAsync(ulong messageId, int sequence, BinaryReader messageReader, TLMethod request, CancellationToken token = default(CancellationToken))
        {
            token.ThrowIfCancellationRequested();
        ...
        private async Task<bool> HandleContainerAsync(ulong messageId, int sequence, BinaryReader messageReader, TLMethod request, CancellationToken token = default(CancellationToken))
        {
            token.ThrowIfCancellationRequested();
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                try
                {
                    if (!await processMessageAsync(innerMessageId, sequence, messageReader, request, token))
                    {
